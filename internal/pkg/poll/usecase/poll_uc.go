...
func (uc *PollUsecase) GetPollQuestions(ctx context.Context, userID int64) (pollQuestions []models.PollQuestion, err error) {
	funcName := "GetPollQuestions"

	err = uc.pollRepo.SetNextPollDT(ctx, userID)
	if err != nil {
		return nil, fmt.Errorf("%s (SetNextPollDT): %w", funcName, err)
	}

	pollQuestions, err = uc.pollRepo.GetPollQuestions(ctx)
	if err != nil {
		return nil, fmt.Errorf("%s (GetPollQuestions): %w", funcName, err)
	}

	return pollQuestions, err
}
